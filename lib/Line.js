            ...
            points.map(function (p, i) {
                ...
                if (pointIconForPoint && pointIconForPoint[i] && pointIconForPoint[i].x === data[i].x && pointIconForPoint[i].y === data[i].y && pointIconForPoint[i].icon !== undefined) {
                    if (pointIconForPoint[i].icon === null) {
                        ...
                        return (React.createElement(react_native_svg_1.ForeignObject, { x: p.x - 10 + shape.dx, y: p.y - 10 - shape.dy }, React.cloneElement(pointIconForPoint[i].icon, { width: 20, height: 20, key: JSON.stringify(p) })));
                    }
                }
                else {
