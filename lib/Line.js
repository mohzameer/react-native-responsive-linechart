...
var Line = React.forwardRef(function Line(props, ref) {
    ...
    var lastSliceStartedArrayInd = -1;
    var pointArrs = [];
    ...
    if (pointArrs.length > 0) {
        pointArrs.forEach(function (element) {
            if (element.length > 0) {
                path = utils_1.svgPath(element, smoothing, tension);
                paths.push(path);
            }
        });
    }
    else {
        path = utils_1.svgPath(points, smoothing, tension);
    }
    var tooltipsAll = [];
    if (tooltipComponent) {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(react_native_svg_1.G, { translateX: viewportOrigin.x, translateY: viewportOrigin.y },
            paths.length > 0 ? paths.map(function (path, index) {
                return React.createElement(react_native_svg_1.Path, { key: index, d: path, fill: "none", strokeLinecap: "round", strokeDasharray: stroke.dashArray.length > 0 ? stroke.dashArray.join(',') : undefined, stroke: stroke.color, strokeWidth: stroke.width, strokeOpacity: stroke.opacity, mask: "url(#Mask)" });
            ...
            points.map(function (p, i) {
                ...
                if (pointIconForPoint && pointIconForPoint[i] && pointIconForPoint[i].x === data[i].x && pointIconForPoint[i].y === data[i].y && pointIconForPoint[i].icon !== undefined) {
                    if (pointIconForPoint[i].icon === null) {
                        return (React.createElement(react_native_svg_1.ForeignObject, { x: p.x - 10 + shape.dx, y: p.y - 10 - shape.dy, key: JSON.stringify(p) + '-' + i }));
                    }
                    else {
                        return (React.createElement(react_native_svg_1.ForeignObject, { x: p.x - 10 + shape.dx, y: p.y - 10 - shape.dy, key: JSON.stringify(p) + '-' + i }, React.cloneElement(pointIconForPoint[i].icon, { width: 20, height: 20 })));
                    }
                }
                else if (ySkipPoint !== undefined && ySkipPoint !== null && data[i].y == ySkipPoint) {
                    return (React.createElement(react_native_svg_1.Rect, { key: JSON.stringify(p) + '-' + i, x: p.x - shape.width / 2 + shape.dx, y: p.y - shape.height / 2 - shape.dy, rx: shape.rx, fill: shape.color, opacity: 0.0, height: shape.height, width: shape.width }));
                }
                else {
                    return (React.createElement(react_native_svg_1.Rect, { key: JSON.stringify(p) + '-' + i, x: p.x - shape.width / 2 + shape.dx, y: p.y - shape.height / 2 - shape.dy, rx: shape.rx, fill: shape.color, opacity: shape.opacity, height: shape.height, width: shape.width, strokeWidth: shape.lineWidth, stroke: shape.lineColor }));
                }
            })),
