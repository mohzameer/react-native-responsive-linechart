...
var Chart = React.memo(React.forwardRef(function (props, ref) {
    ...
    var scrollXValue = function (origin, x) {
        if (dataDimensions) {
            var factorX = viewport.size.width / dataDimensions.width;
            setPanX(origin.x - x * factorX);
            offset.x.setValue(lodash_clamp_1.default(origin.x - x, xDomain.min, xDomain.max - viewport.size.width));
        }
    };
    ...
    React.useImperativeHandle(ref, function () { return ({ setViewportOrigin: setViewportOrigin, scrollXValue: scrollXValue }); });
